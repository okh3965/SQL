...
-- VIEW : 한 개 혹은 복수 개의 테이블을 기반으로 함, 실제 데이터는 갖고있지않음.
-- VIEW 생성을 위해서는 CREATE VIEW 권한이 필요
-- system으로 로그인
GRANT CREATE VIEW TO C##KHOH;

-- Simple VIEW
-- 단일 테이블 혹으느 뷰를 기반으로 생성
-- 제약조건 위반이 없다면 INSERT, UPDATE, DELETE 가능
-- employee 테이블로부터 department_id가 10인 사람들만 VIEW로 생성
-- 기반 테이블 생성
CREATE TABLE emp_10
...
VALUES(2, '살인자의 기억법', 2); -- pub_date는 default 입력된다.
...
-- s_emp 테이블 employee_id 컬럼에 UNIQUE 인덱스 생성해 보기
CREATE UNIQUE INDEX s_emp_id_pk -- index name
...
SELECT my_seq.NEXTVAL FROM dual;    -- 새 값의 추출
